...
object OCRPipeline {
    ...
    val batchSize = 3
    fun extractTexts(bitmap: Bitmap): Pair<DBPostProcess.DetectionResult, Int> {
        ...
        for (i in 0 until totalBatches - 1 ) {
            ...
            for (j in start until max(start + batchSize, det_result.boxes.size - 1)) {
                logger.DEBUG("current file index : $j")
            }
        }
        return Pair<DBPostProcess.DetectionResult, Int>(det_result, 1)
    }
}